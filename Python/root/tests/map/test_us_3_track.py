...
class TestUs3Tracks(unittest.TestCase):
    def test_us_viii_track_has_8_spaces(self):
        self.assertEqual(len(us_viii_track), 8)

    def test_us_xv_track_has_5_spaces(self):
        self.assertEqual(len(us_xv_track), 5)
    ...
    def test_xv_track_progression(self):
        expected_track_numbers = [4, 3, 2, 1, 0]

        actual_track_numbers = [space.track_number for space in us_xv_track]

        self.assertEqual(actual_track_numbers, expected_track_numbers)

    def test_all_spaces_start_german_controlled(self):
        all_spaces = us_viii_track + us_xv_track[1:]
